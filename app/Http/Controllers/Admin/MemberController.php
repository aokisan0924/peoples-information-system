...
class MemberController extends Controller
{
    ...
    public function showEncrypted(string $encrypted) {
        $memberId = decrypt($encrypted);

        $member = Member::where('id', $memberId)->findOrFail($memberId);
        $basicInfoData = [
            'id' => $member->id,
            'username' => $member->username,
            'firstName' => $member->firstName,
        ...
        $branchService = BranchService::where('memberId', $memberId)->findOrFail($memberId);
        $branServiceData = [
            'branchService' => $branchService->branchService,
            'subBranch' => $branchService->subBranch
        ];

        $afpInfo = AFPInfo::where('memberId', $memberId)->findOrFail($memberId);
        $afpData = [
            'afpsn' => $afpInfo->afpsn,
            'rank' => $afpInfo->rank,
        ...
        $spouseInfo = SpouseInfo::where('memberId', $memberId)->findOrFail($memberId);
        $spouseData = [
        ...
        $parentsInfo = ParentsInfo::where('memberId', $memberId)->findOrFail($memberId);
        $parentsData = [
        ...
        $identificationInfo = IdentificationInfo::where('memberId', $memberId)->findOrFail($memberId);
        $identificationData = [
        ...
        $emergencyInfo = EmergencyContact::where('memberId', $memberId)->findOrFail($memberId);
        $emergencyData = [
        ...
        // Build ledger-style transactions for THIS time deposit
        $transactions   = [];
        $runningBalance = 0.0;
        ...
        // Sort transactions by date ASC (in case)
        usort($transactions, function ($a, $b) {
            return strcmp($a['date'] ?? '', $b['date'] ?? '');
        });

        // Totals for this TD using relations (no extra queries)
        ...
        // Accumulate overall member totals
        $totalPrincipal    += $principal;
        $totalCurrBalance  += $currBalance;
        $totalAvailInterest += $availInterest;

        // Summary for this specific time deposit
        $summaryDisplay = [
            'timeDepositId'     => $td->id,
            ...
            'startDate'         => $td->startDate
                ? $td->startDate->format('d M y')  // 11 Nov 25
                : null,
        ...
        // Ledger rows formatted for frontend
        $transactionRows = array_map(function (array $t) {
            ...
            return [
                ...
                'type'        => $t['type'] ?? null, // 'credit' or 'debit'
                'debit'       => ($t['debit'] ?? 0.0) > 0
        ...
        // Overall summary for all TD of this member
    ...
    public function updateBasicInfo(Request $request, $encrypted) {
        try {
            $memberId = decrypt($encrypted);
    ...
    public function updateBranchService(Request $request, $encrypted) {
        try {
            $memberId = decrypt($encrypted);
            $member = Member::findOrFail($memberId);
        } catch (\Throwable $e) {
    ...
    public function updateAfpInfo(Request $request, $encrypted) {
        try {
            $memberId = decrypt($encrypted);
            $member   = Member::findOrFail($memberId);
        } catch (\Throwable $e) {
            return response()->json([
                'success' => false,
                'message' => 'Member not found or invalid reference.',
            ], 404);
        }
    ...
    public function updateSpouseInfo(Request $request, $encrypted) {
        try {
    ...
    public function updateParentsInfo(Request $request, $encrypted) {
        try {
            $id     = decrypt($encrypted);
            $member = Member::findOrFail($id);
        } catch (\Throwable $e) {
    ...
    public function updateIdentificationInfo(Request $request, $encrypted) {
        try {
            $id     = decrypt($encrypted);
            $member = Member::findOrFail($id);
        } catch (\Throwable $e) {
    ...
    public function updateEmergencyInfo(Request $request, $encrypted) {
        try {
            $id     = decrypt($encrypted);
            $member = Member::findOrFail($id);
        } catch (\Throwable $e) {
    ...
    public function updateDependents(Request $request, $encrypted) {
        try {
            $memberId = decrypt($encrypted);
            $member   = Member::findOrFail($memberId);
        } catch (\Throwable $e) {
