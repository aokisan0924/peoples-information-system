...
import { 
    ...
    FileText, CheckCircle2, AlertCircle, Clock, XCircle, ArrowRight, X
} from "lucide-react";
...
export default function Loan() {
    ...
    // --- COMPUTATION LOGIC ---
    useEffect(() => {
        // --- FIX: Prevent non-creators from triggering computation ---
        if (!canCreate) return; 
        ...
        .catch(() => {
            // Only toast if modal is actually open, otherwise silent
        ...
        <AdminSidebarLayout>
            ...
            <div className="space-y-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    {/* NEW LOAN BUTTON: Hidden if role is restricted */}
                    {canCreate && (
                        <button 
                    ...
                    <div className="hidden sm:block overflow-x-auto">
                        <table className="w-full text-left text-sm">
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-white/5 text-slate-700 dark:text-slate-200">
                                {loading ? (
                                    <tr><td colSpan="7" className="px-6 py-12 text-center text-slate-500 dark:text-slate-400"><Loader2 className="h-6 w-6 animate-spin mx-auto mb-2"/>Loading data...</td></tr>
                                ) : loans.length === 0 ? (
                                    <tr><td colSpan="7" className="px-6 py-12 text-center text-slate-500 dark:text-slate-400">No records found.</td></tr>
                                ) : (
                                    loans.map((row, idx) => (
            ...
            {/* MODAL (Only renders if authorized) */}
            <AnimatePresence>
                {showModal && canCreate && (
