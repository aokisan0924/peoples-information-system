                ...
                <div className="space-y-6">
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-white dark:border-white/10 dark:bg-white/5 p-4 shadow-sm transition-colors">
                        <div className="flex flex-col lg:flex-row gap-4">
                            ...
                            {/* Date & Filter Controls - Stack on mobile, inline on desktop */}
                                        ...
                                        rows.map((row) => (
                                            <tr key={row.id} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                                                ...
                                                <td className="px-6 py-4 text-xs text-slate-500 dark:text-slate-400">
                                                    <div><span className="opacity-50">Start:</span> {row.startDate}</div>
                                                    <div><span className="opacity-50">End:</span> {row.maturityDate}</div>
                                                </td>
                                                <td className="px-6 py-4 text-center">
                            ...
                            {rows.map((row) => (
                                <div key={row.id} className="p-4 space-y-3">
                                    ...
                                    <div className="grid grid-cols-2 gap-2 text-xs border-t border-slate-100 dark:border-white/5 pt-3">
                                        <div>
                                            <div className="text-[10px] uppercase opacity-60 dark:text-slate-400 mb-0.5">Principal</div>
                                            <div className="font-mono text-emerald-600 dark:text-emerald-400 text-sm font-medium">{asMoney(row.principal)}</div>
                                        </div>
                                        <div className="text-right">
                                            <div className="text-[10px] uppercase opacity-60 dark:text-slate-400 mb-0.5">Maturity</div>
                                            <div className="font-mono text-amber-600 dark:text-amber-400 font-bold text-sm">{asMoney(row.maturityValue)}</div>
                                        </div>
                                    </div>
                                    <Link href={route('admin.time.member', row.id)} className="flex items-center justify-center w-full py-2.5 rounded-lg bg-emerald-600 text-white text-xs font-semibold shadow-md active:scale-95 transition-transform">
                                        View Details
                                    </Link>
                                </div>
                            ))}
                        ...
                        <motion.div className="fixed inset-0 z-50 flex items-center justify-center px-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                            ...
                            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                                ...
                                <form onSubmit={submitForm} className="p-6 space-y-5 overflow-y-auto">
                                    ...
                                    <div className="pt-2">
                                        <button type="submit" disabled={submitting} className="w-full py-3 rounded-xl text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-500 shadow-lg shadow-emerald-500/20 transition-all disabled:opacity-50 active:scale-95">
