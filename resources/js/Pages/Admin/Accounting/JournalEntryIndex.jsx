...
import { Banknote, BookOpenCheck, Briefcase, Building2, CalendarDays, CreditCard, Landmark, Search, Smartphone, User, WalletCards } from "lucide-react";
...
const sourceMeta = {
    ...
    ppe: { label: "PPE depreciation", icon: Briefcase },
    membership: { label: "Membership fee", icon: WalletCards },
    capital: { label: "Share capital", icon: WalletCards },
    savings: { label: "Savings deposit", icon: WalletCards },
    memcap: { label: "Member onboarding", icon: WalletCards },
};

export default function JournalEntryIndex({ batches, filters, sourceTypes = [] }) {
    ...
    const applyFilters = (event) => {
        ...
        router.get(
            ...
            { search: search.trim(), status, source_type: sourceType },
    ...
    return (
        <AdminSidebarLayout>
            <Head title="Journal Review Queue" />
                    ...
                    <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
                        <div>
                            ...
                            <h1 className="text-2xl font-black uppercase tracking-tight text-white sm:text-3xl">
                                Journal Review Batches
                            </h1>
                            <p className="mt-2 max-w-2xl text-sm text-slate-400">
                                Review source transactions before they are posted to the General Ledger.
                            </p>
                        </div>

                        <form onSubmit={applyFilters} className="grid w-full gap-3 sm:grid-cols-2 lg:max-w-4xl lg:grid-cols-[minmax(0,1fr)_12rem_12rem_auto]">
                            ...
                            >
                                ...
                                {sourceTypes.map((type) => <option key={type} value={type}>{sourceMeta[type]?.label ?? type}</option>)}
                            </select>
                            <select
                                value={status}
                ...
                <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-xl dark:border-white/10 dark:bg-slate-900/70">
                        ...
                        <table className="min-w-full text-left text-sm">
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                                {batches.data.map((batch) => {
                                    const SourceIcon = sourceMeta[batch.source_type]?.icon ?? WalletCards;
                                    return (
                                    <tr key={`${batch.source_type}-${batch.batch_reference}`} className="transition hover:bg-slate-50 dark:hover:bg-white/[0.03]">
                                        ...
                                        <td className="px-5 py-4">
                                            <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-600 dark:text-slate-300">
                                                <SourceIcon className="h-3.5 w-3.5 text-emerald-500" /> {sourceMeta[batch.source_type]?.label ?? batch.source_type}
                                            </span>
                                        </td>
                                        <td className="px-5 py-4">
                                        ...
                                        <td className="px-5 py-4 text-right">
                                            <Link
                                                href={route("admin.accounting.journal-entries.show", { batchReference: batch.batch_reference, source_type: batch.source_type, source_record_id: batch.source_record_id, branch: batch.branch })}
                    ...
                    {batches.data.length === 0 && (
                        <div className="px-6 py-16 text-center">
                            ...
                            <p className="mt-3 font-bold text-slate-700 dark:text-slate-200">No journal batches found.</p>
