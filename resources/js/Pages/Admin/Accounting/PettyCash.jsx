    ...
    return (
        <AdminSidebarLayout>
                ...
                <div className="bg-slate-900 rounded-[2rem] p-8 shadow-2xl border border-white/5">
                    <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6">
                        <div className="space-y-2">
                            <h1 className="text-3xl font-black text-white tracking-tight uppercase">Petty Cash <span className="text-emerald-400">Daily</span></h1>
                        ...
                        <thead className="bg-white/5 text-[10px] font-black uppercase text-slate-400 tracking-widest border-b border-white/5">
                            <tr>
                                <th className="px-6 py-5 w-10 text-center">
                                    <input type="checkbox" className="rounded border-white/10 bg-white/5" onChange={(e) => {
                                ...
                                return (
                                    <tr key={record.id} className="hover:bg-emerald-400/5 transition-colors group text-white text-sm">
                                        <td className="px-6 py-4 text-center">
                                            {record.is_posted && <input type="checkbox" className="rounded border-white/10 bg-white/5 text-emerald-500" checked={selectedVouchers.includes(record.id)} onChange={() => setSelectedVouchers(prev => prev.includes(record.id) ? prev.filter(i => i !== record.id) : [...prev, record.id])} />}
                                        </td>
                                        ...
                                        <td className="px-6 py-4 font-mono text-[10px] font-black text-indigo-400 uppercase">
                                            {isEditing ? <input type="text" value={editForm.orNumber || ''} onChange={e => setEditForm({...editForm, orNumber: e.target.value})} className="bg-slate-900 border-emerald-500/50 rounded w-full p-1" /> : (record.orNumber || '—')}
                                        </td>
            ...
            {selectedVouchers.length > 0 && (
                <div className="fixed bottom-10 left-1/2 -translate-x-1/2 bg-slate-900 text-white px-8 py-4 rounded-2xl shadow-2xl flex items-center gap-6 z-[150] border border-white/10">
                    <div className="flex items-center gap-3"><div className="p-2 bg-emerald-500 rounded-lg text-slate-900"><Printer size={18}/></div><span className="font-bold text-sm">{selectedVouchers.length} Selected</span></div>
                    <button onClick={() => window.open(route('admin.accounting.petty.print', { ids: selectedVouchers.join(','), perPage: 3 }))} className="bg-white/10 hover:bg-white/20 px-4 py-2 rounded-lg font-black text-[10px] uppercase">Print Batch (3/Page)</button>
                    <button onClick={() => setSelectedVouchers([])} className="p-2 text-slate-400 hover:text-white"><X size={20}/></button>
                </div>
            )}
            ...
            {showLogModal && (
                <div className="fixed inset-0 z-[100] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-slate-900 border border-white/10 rounded-[2rem] w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
                        <div className="flex justify-between items-center p-6 border-b border-white/5">
                            <h2 className="text-xl font-black text-white uppercase tracking-tight flex items-center gap-2"><ReceiptText className="text-emerald-400"/> New Transactions</h2>
                            ...
                            {logs.map((log, idx) => (
                                <div key={log.id} className="bg-white/5 p-6 rounded-2xl relative border border-white/5 text-white">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                        <div><label className="text-[10px] font-black uppercase text-emerald-400 tracking-widest">OR Number</label><input type="text" placeholder="Optional" value={log.orNumber} onChange={e => handleLogChange(log.id, 'orNumber', e.target.value.toUpperCase())} className="w-full bg-slate-800 border-0 rounded-xl mt-1 text-sm uppercase font-mono focus:ring-emerald-500"/></div>
