        ...
        <>
            ...
            <AdminSidebarLayout>
                <div className="space-y-6">
                    ...
                    {/* STATS CARDS - Grid fits 5 items on XL screens */}
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
                        ...
                        <StatCard label="Total Loan Income" value={dashboardSummary.totalLoanIncome} prefix="₱" icon={TrendingUp} color="rose" />
                    </div>
                            ...
                            <div className="flex items-center justify-between mb-6">
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500 dark:text-slate-400">
                                        Capital, Savings, Time Deposits & Loan Income (6 Months)
                                    </p>
                                </div>
                            </div>
                            <div className="h-[350px] w-full">
                                <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                        ...
                                        <Area name="Loan Income" type="monotone" dataKey="loan_income" stroke="#f43f5e" strokeWidth={3} fillOpacity={1} fill="url(#colorLoan)" stackId="1" />
                                    </AreaChart>
                                </ResponsiveContainer>
                            </div>
                        </div>
                                ...
                                {branchData.length > 0 ? (
                                    <ResponsiveContainer width="100%" height="100%">
                                        <PieChart>
                                            ...
                                            <Tooltip contentStyle={{ backgroundColor: 'var(--tooltip-bg)', borderColor: 'var(--tooltip-border)', color: 'var(--tooltip-text)', borderRadius: '12px' }} />
...
// --- STAT CARD COMPONENT (UPDATED FOR SMALLER TEXT) ---
function StatCard({ label, value, prefix = "", icon: Icon, color }) {
    ...
    return (
        <div className="rounded-3xl border border-slate-200 bg-white dark:border-white/10 dark:bg-white/5 p-5 shadow-sm hover:shadow-md transition-all group">
            <div className="flex items-start justify-between">
                <div>
                    {/* REDUCED LABEL SIZE: text-[11px] */}
                    <p className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">{label}</p>
                    
                    {/* REDUCED VALUE SIZE: text-xl sm:text-2xl */}
                    <div className="mt-1 text-xl sm:text-2xl font-bold text-slate-900 dark:text-white font-mono tracking-tight">
                        {prefix}<CountUp end={value} duration={1.5} separator="," decimals={value % 1 !== 0 ? 2 : 0} />
                    </div>
                </div>
                {/* REDUCED ICON PADDING AND SIZE */}
                <div className={`p-2.5 rounded-2xl ${colorStyles[color]} transition-transform group-hover:scale-110`}>
                    <Icon size={20} />
                </div>
            </div>
        </div>
